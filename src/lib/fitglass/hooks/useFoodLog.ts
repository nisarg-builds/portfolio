...
export function useFoodLog() {
  ...
  const isLoading = useFitGlassStore((s) => s.isLoadingToday);
  const addFoodEntry = useFitGlassStore((s) => s.addFoodEntry);
  const removeFoodEntry = useFitGlassStore((s) => s.removeFoodEntry);
  const loadTodayEntries = useFitGlassStore((s) => s.loadTodayEntries);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        useFitGlassStore.setState({ todayEntries: entries });
      },
      (error) => {
