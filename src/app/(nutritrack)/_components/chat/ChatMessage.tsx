...
function UserBubble({ message }: ChatMessageProps) {
  ...
  return (
    <div className="flex max-w-[80%] items-end gap-2">
      ...
      {/* User avatar */}
      <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-nt-accent bg-nt-surface text-[10px] font-bold text-nt-text">
        {initial}
      </div>
    </div>
  );
  ...
  return (
    <div className="flex max-w-[85%] items-end gap-2">
      ...
      <div className="flex flex-col gap-1.5">
        {message.isLoading ? (
          <div className="rounded-[14px] rounded-bl-[4px] border border-nt-border bg-nt-chat-ai px-3.5 py-2.5">
            <LoadingDots />
          </div>
        ) : (
          <>
            <div className="rounded-[14px] rounded-bl-[4px] border border-nt-border bg-nt-chat-ai px-3.5 py-2.5">
              <p
                className={cn('text-sm', message.isError ? 'text-nt-danger' : 'text-nt-text')}
              >
                {message.text}
              </p>
            </div>
