...
import type { ComputedTargets } from '@/lib/nutritrack/models';
...
function Stat({ label, value, accent }: StatProps) {
  return (
    <div>
      <p
        className={`text-2xl font-bold tracking-tight ${accent ? 'text-nt-accent' : 'text-nt-text'}`}
      >
        {value}
      </p>
      <p className="mt-0.5 text-xs font-medium uppercase tracking-widest text-nt-text-soft">
        {label}
      </p>
    </div>
  );
