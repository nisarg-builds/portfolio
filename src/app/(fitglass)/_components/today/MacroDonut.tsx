...
const COLORS = {
  protein: 'var(--color-fg-protein)',
  carbs: 'var(--color-fg-carbs)',
  fat: 'var(--color-fg-fat)',
};
  ...
  return (
    <Card className="flex items-center gap-4" aria-label={`Macro breakdown: ${Math.round(proteinG)}g protein, ${Math.round(carbsG)}g carbs, ${Math.round(fatG)}g fat`}>
      ...
      <div className="flex flex-1 gap-4">
        <MacroColumn label="Protein" value={proteinG} color="bg-fg-protein" />
        <MacroColumn label="Carbs" value={carbsG} color="bg-fg-carbs" />
        <MacroColumn label="Fat" value={fatG} color="bg-fg-fat" />
      </div>
    </Card>
  );
}
...
function MacroColumn({ label, value, color }: { label: string; value: number; color: string }) {
  return (
    <div className="flex flex-col items-center gap-0.5">
      ...
      <span className="text-sm font-semibold text-fg-text">{Math.round(value)}g</span>
      <span className="text-xs text-fg-text-soft">{label}</span>
    </div>
  );
}
