...
import type { AIFoodItem } from '@/lib/fitglass/models/chat';
import { aiItemToFoodEntry } from '@/lib/fitglass/utils/converters';
...
export function FoodCard({ food }: FoodCardProps) {
  ...
  const addFoodEntry = useFitGlassStore((s) => s.addFoodEntry);
  ...
  return (
    <div className="rounded-xl border border-fg-border bg-fg-card p-3">
      {/* Header: name + log button */}
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-medium text-fg-text">{food.name}</p>
        <motion.button
          whileTap={{ scale: 0.9 }}
          onClick={handleLog}
          disabled={logged}
          className="shrink-0 rounded-lg bg-fg-accent px-2.5 py-1 text-xs font-medium text-white disabled:opacity-50"
      ...
      <div className="mt-2 flex flex-wrap gap-1.5">
        <MicroBadge
          ...
          value={food.cal}
          unit=""
          color="bg-fg-surface text-fg-text"
        />
        <MicroBadge
          label="P"
          value={food.protein}
          unit="g"
          color="bg-fg-accent-light text-fg-protein"
        />
        <MicroBadge
          label="C"
          value={food.carbs}
          unit="g"
          color="bg-fg-carbs/10 text-fg-carbs"
        />
        <MicroBadge
          label="F"
          value={food.fat}
          unit="g"
          color="bg-fg-fat/10 text-fg-fat"
        />
      </div>
      ...
      {hasMicros && (
        <>
          <button
            ...
            className="mt-2 text-xs font-medium text-fg-accent"
            data-cursor="interactive"
          >
